...
import time,string,os,struct,sys
...
class sensor(gr.top_block):
	  ...
	  self.min_freq = Freq.value-(200*10**3) # same as that of the transmitter bandwidth ie 6MHZ approx for a given value of decimation line option any more
	  self.max_freq = Freq.value+(200*10**3)
...
class trans_init():
	  ...
	  parser.add_option("-d", "--decim", type="intx", default=128, help="set decimation to DECIM [default=%default]")	# changed default value(16) to 128
...
def run_transmitter(tb,norestart,carrier_map):  
      tb.start()
...
def Start_transmitter(Freq,norestart):
      ...
      while 1:  
	    carrier_map=sense_loop(tb1)       
	    n=run_transmitter(tb,norestart,carrier_map)
